...
class upload {
		...
		if(in_array($typeFile, $type))
		{
			...
			if(count($extension) <= 2 && in_array(strtolower(end($extension)), $extensions))
			{
				...
				if($sizeFile < $max_size)
				{
					...
					if(move_uploaded_file($tmpFile, $_POST['titre'].'/'.$nameFile))
						echo '<center><h8 class="alert alert-success">
						$_POST["titre"].'/'.$nameFile
						This picture is uploaded!</h8></center>';
				}
				else 
				{
    ...
    public function path(string $name) : string{
			...
			$upload=$name.'/'.$_FILES[$this->file]['name'];
		echo "<h1>'.$upload.'</h1>";
			return $upload;
